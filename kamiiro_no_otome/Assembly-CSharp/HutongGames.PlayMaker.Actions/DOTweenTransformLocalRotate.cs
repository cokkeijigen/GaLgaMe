...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLocalRotate : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalRotate(to.Value, duration.Value, rotateMode);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalRotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
				break;
			}
			...
			{
				tween.From(setReverseRelative.Value);
			}
			if (startEvent != null)
			{
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Local Rotate");
			}
			if (finishImmediately.Value)
			{
				Finish();
