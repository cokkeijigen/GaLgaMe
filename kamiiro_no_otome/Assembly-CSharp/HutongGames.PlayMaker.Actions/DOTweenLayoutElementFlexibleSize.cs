...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLayoutElementFlexibleSize : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<LayoutElement>().DOFlexibleSize(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Layout Element Flexible Size");
			}
			if (finishImmediately.Value)
			{
				Finish();
