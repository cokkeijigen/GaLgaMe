...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbody2DMove : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMove(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMove(toGameObject.Value.transform.position, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody2D Move");
			}
			if (finishImmediately.Value)
			{
				Finish();
