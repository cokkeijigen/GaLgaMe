...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformScaleX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleX(to.Value, duration.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleX(toGameObject.Value.transform.localScale.x, duration.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Scale X");
			}
			if (finishImmediately.Value)
			{
				Finish();
