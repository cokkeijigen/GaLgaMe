...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraNearClipPlane : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DONearClipPlane(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Camera NearClipPlane");
			}
			if (finishImmediately.Value)
			{
				Finish();
