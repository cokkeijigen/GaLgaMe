...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformPunchPosition : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOPunchPosition(punch.Value, duration.Value, vibrato.Value, elasticity.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Punch Position");
			}
			if (finishImmediately.Value)
			{
				Finish();
