...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenScrollRectVerticalNormalizedPos : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<ScrollRect>().DOVerticalNormalizedPos(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween ScrollRect Vertical NormalizedPos");
			}
			if (finishImmediately.Value)
			{
				Finish();
