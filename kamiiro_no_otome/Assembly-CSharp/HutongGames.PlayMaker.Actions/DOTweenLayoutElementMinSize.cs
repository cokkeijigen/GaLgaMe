...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLayoutElementMinSize : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<LayoutElement>().DOMinSize(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Layout Element Min Size");
			}
			if (finishImmediately.Value)
			{
				Finish();
