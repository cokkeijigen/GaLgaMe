...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraShakeRotation : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOShakeRotation(duration.Value, strength.Value, vibrato.Value, randomness.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Camera Shake Rotation");
			}
			if (finishImmediately.Value)
			{
				Finish();
