...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformMoveZ : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveZ(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveZ(toGameObject.Value.transform.position.z, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Move Z");
			}
			if (finishImmediately.Value)
			{
				Finish();
