...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSpriteRendererFade : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<SpriteRenderer>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Sprite Renderer Fade");
			}
			if (finishImmediately.Value)
			{
				Finish();
