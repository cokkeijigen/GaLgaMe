...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformScale : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScale(to.Value, duration.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScale(toGameObject.Value.transform.localScale, duration.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Scale");
			}
			if (finishImmediately.Value)
			{
				Finish();
