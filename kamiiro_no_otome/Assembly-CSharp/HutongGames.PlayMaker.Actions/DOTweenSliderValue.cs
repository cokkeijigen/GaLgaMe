...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSliderValue : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Slider>().DOValue(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Slider Value");
			}
			if (finishImmediately.Value)
			{
				Finish();
