...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLocalMoveX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveX(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Local Move X");
			}
			if (finishImmediately.Value)
			{
				Finish();
