...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformShakePosition : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOShakePosition(duration.Value, strength.Value, vibrato.Value, randomness.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Shake Position");
			}
			if (finishImmediately.Value)
			{
				Finish();
