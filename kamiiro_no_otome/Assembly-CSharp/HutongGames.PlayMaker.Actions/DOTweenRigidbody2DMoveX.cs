...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbody2DMoveX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveX(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody2D Move X");
			}
			if (finishImmediately.Value)
			{
				Finish();
