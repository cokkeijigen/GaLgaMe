...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformAnchorPosX : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOAnchorPosX(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RectTransform Anchor Pos X");
			}
			if (finishImmediately.Value)
			{
				Finish();
