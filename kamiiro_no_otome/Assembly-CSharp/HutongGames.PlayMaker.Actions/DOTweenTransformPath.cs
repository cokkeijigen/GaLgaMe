...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformPath : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case LookAt.nothing:
				tween = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation);
				break;
			case LookAt.position:
				tween = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation).SetLookAt(lookAtPosition.Value, forwardDirection.Value, up.Value);
				break;
			case LookAt.target:
				tween = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation).SetLookAt(lookAtTarget.Value.transform, forwardDirection.Value, up.Value);
				break;
			case LookAt.ahead:
				tween = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation).SetLookAt(lookAhead.Value, forwardDirection.Value, up.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Path");
			}
			if (finishImmediately.Value)
			{
				Finish();
