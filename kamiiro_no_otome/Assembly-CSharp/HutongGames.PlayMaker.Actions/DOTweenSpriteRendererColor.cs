...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSpriteRendererColor : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<SpriteRenderer>().DOColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Sprite Renderer Color");
			}
			if (finishImmediately.Value)
			{
				Finish();
