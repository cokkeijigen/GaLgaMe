...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyLookAtPosition : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOLookAt(position.Value, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Look At Position");
			}
			if (finishImmediately.Value)
			{
				Finish();
