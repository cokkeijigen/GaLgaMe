...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformMoveX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveX(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Move X");
			}
			if (finishImmediately.Value)
			{
				Finish();
