...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTextText : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Text>().DOText(to.Value, duration.Value, richTextEnabled.Value, scrambleMode, scrambleChars.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Text Text");
			}
			if (finishImmediately.Value)
			{
				Finish();
