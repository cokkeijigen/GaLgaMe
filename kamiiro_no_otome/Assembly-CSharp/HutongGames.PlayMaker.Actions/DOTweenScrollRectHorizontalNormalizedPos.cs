...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenScrollRectHorizontalNormalizedPos : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<ScrollRect>().DOHorizontalNormalizedPos(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween ScrollRect Horizontal NormalizedPos");
			}
			if (finishImmediately.Value)
			{
				Finish();
