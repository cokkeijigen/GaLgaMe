...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraFarClipPlane : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOFarClipPlane(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Camera Far Clip Plane");
			}
			if (finishImmediately.Value)
			{
				Finish();
