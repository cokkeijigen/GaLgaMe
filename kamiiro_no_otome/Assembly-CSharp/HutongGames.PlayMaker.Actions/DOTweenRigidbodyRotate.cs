...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyRotate : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, rotateMode);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DORotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Rotate");
			}
			if (finishImmediately.Value)
			{
				Finish();
