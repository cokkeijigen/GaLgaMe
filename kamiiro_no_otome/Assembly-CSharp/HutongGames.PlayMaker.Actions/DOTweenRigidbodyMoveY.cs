...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyMoveY : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMoveY(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMoveY(toGameObject.Value.transform.position.y, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Move Y");
			}
			if (finishImmediately.Value)
			{
				Finish();
