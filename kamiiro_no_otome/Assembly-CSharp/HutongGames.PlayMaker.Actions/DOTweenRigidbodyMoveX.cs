...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyMoveX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMoveX(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Move X");
			}
			if (finishImmediately.Value)
			{
				Finish();
