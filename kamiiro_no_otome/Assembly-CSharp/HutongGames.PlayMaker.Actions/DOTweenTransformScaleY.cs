...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformScaleY : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleY(to.Value, duration.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleY(toGameObject.Value.transform.localScale.y, duration.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Scale Y");
			}
			if (finishImmediately.Value)
			{
				Finish();
