...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialOffsetProperty : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOOffset(to.Value, property.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Material Offset");
			}
			if (finishImmediately.Value)
			{
				Finish();
