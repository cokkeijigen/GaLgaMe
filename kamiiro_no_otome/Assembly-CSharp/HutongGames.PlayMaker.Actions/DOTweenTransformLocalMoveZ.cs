...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLocalMoveZ : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveZ(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveZ(toGameObject.Value.transform.position.z, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Local Move Z");
			}
			if (finishImmediately.Value)
			{
				Finish();
