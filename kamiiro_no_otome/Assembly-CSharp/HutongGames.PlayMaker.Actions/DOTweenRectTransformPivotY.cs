...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformPivotY : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOPivotY(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RectTransform PivotY");
			}
			if (finishImmediately.Value)
			{
				Finish();
