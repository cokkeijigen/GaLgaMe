...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformMoveY : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveY(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveY(toGameObject.Value.transform.position.y, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Move Y");
			}
			if (finishImmediately.Value)
			{
				Finish();
