...
public class PlayerScenarioDataManager : SerializedMonoBehaviour
{
	public static WorldEventCheckData CheckWorldAccessPointCurrentEvent(string pointName)
	{
		...
		if (!flag)
		{
			flag = false;
			dictionary.Clear();
			dictionary = CheckCommonScenarioFlag(scenarioFlagData, isWorldMap: true);
			...
			if (scenarioFlagData.needFollowHeroineNum != 9 && scenarioFlagData.needFollowHeroineNum != 0)
			{
				...
				{
					flag3 = true;
					Debug.Log("要同行ヒロインが同行していない：" + pointName + "／必要なヒロインID：" + scenarioFlagData.needFollowHeroineNum);
					switch (scenarioFlagData.needFollowHeroineNum)
					{
					case 1:
						result.disablePointTerm = "dialogWorldMapDisable_RequiredLucy";
	...
	public static WorldEventCheckData CheckDungeonCurrentEvent(string pointName)
	{
		WorldEventCheckData result = default(WorldEventCheckData);
		ScenarioFlagData scenarioFlagData = null;
		Dictionary<string, bool> dictionary = new Dictionary<string, bool>();
		...
		bool flag2 = false;
		bool flag3 = false;
		List<ScenarioFlagData> list = GameDataManager.instance.sceneGarellyDataBase.sceneFlagNameData.scenarioFlagDataList.Where((ScenarioFlagData data) => data.scenarioLocationName == pointName && data.locationType == "dungeon").ToList();
		List<ScenarioFlagData> list2 = new List<ScenarioFlagData>(list);
		for (int num = list2.Count - 1; num >= 0; num--)
		{
			if (PlayerFlagDataManager.scenarioFlagDictionary[list[num].scenarioName])
		...
		if (!flag && flag4)
		{
			...
			Debug.Log("ダンジョンイベント発生あり：" + pointName + "／シナリオ名：" + scenarioFlagData.scenarioName + text);
		}
		else
		{
			result.currentScenarioName = "";
			SetUnachievedFlagDebugLog(dictionary, "dungeon", pointName);
		}
		return result;
	}

	public static WorldEventCheckData CheckWorldAccessPointCurrentTalkEvent(string pointName)
	{
		WorldEventCheckData result = default(WorldEventCheckData);
		ScenarioFlagData scenarioFlagData = null;
		Dictionary<string, bool> dictionary = new Dictionary<string, bool>();
	...
	public static LocalEventCheckData CheckLocalPlaceCurrentEvent(string placeName)
	{
	...
	public static InDoorEventCheckData CheckInDoorCurrentEvent(string placeName)
	{
	...
	public static InDoorEventCheckData CheckInDoorCurrentTalkEvent(string placeName, string characterName)
	{
		InDoorEventCheckData result = default(InDoorEventCheckData);
		ScenarioFlagData scenarioFlagData = null;
		Dictionary<string, bool> dictionary = new Dictionary<string, bool>();
	...
	private static Dictionary<string, bool> CheckCommonScenarioFlag(ScenarioFlagData checkScenarioFlagData, bool isWorldMap)
	{
		...
		if (isWorldMap)
		{
			int num3 = component.GetNeedAccessDay(PlayerDataManager.currentAccessPointName, checkScenarioFlagData.scenarioLocationName);
			int num4 = component.GetNeedAccessTime(PlayerDataManager.currentAccessPointName, checkScenarioFlagData.scenarioLocationName);
			if (num4 >= 4)
			{
