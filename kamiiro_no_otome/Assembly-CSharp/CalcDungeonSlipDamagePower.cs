...
[AddComponentMenu("")]
public class CalcDungeonSlipDamagePower : StateBehaviour
{
	...
	public override void OnStateBegin()
	{
		...
		case "p":
			...
			{
				float num3 = (float)PlayerStatusDataManager.playerAllMaxHp * 0.02f;
		...
		case "e":
			if ((from ano in PlayerBattleConditionManager.enemyBadState[0].Select((PlayerBattleConditionManager.MemberBadState con, int index) => new
				...
				select ano.Index).ToList().Any())
			{
				float num = (float)PlayerStatusDataManager.enemyAllMaxHp * 0.02f;
