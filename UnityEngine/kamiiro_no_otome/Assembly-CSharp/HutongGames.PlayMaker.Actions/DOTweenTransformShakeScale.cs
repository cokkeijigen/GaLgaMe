...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformShakeScale : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOShakeScale(duration.Value, strength.Value, vibrato.Value, randomness.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Shake Scale");
			}
			if (finishImmediately.Value)
			{
				Finish();
