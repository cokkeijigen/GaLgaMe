...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformScaleZ : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleZ(to.Value, duration.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleZ(toGameObject.Value.transform.localScale.z, duration.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Scale Z");
			}
			if (finishImmediately.Value)
			{
				Finish();
