...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialVectorProperty : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOVector(to.Value, property.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Material Vector Property");
			}
			if (finishImmediately.Value)
			{
				Finish();
