...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyMove : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMove(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMove(toGameObject.Value.transform.position, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Move");
			}
			if (finishImmediately.Value)
			{
				Finish();
