...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class SetVelocity : ComponentAction<Rigidbody>
	{
		...
		private void DoSetVelocity()
		{
			...
			{
				Vector3 vector = ((!this.vector.IsNone) ? this.vector.Value : ((space == Space.World) ? base.rigidbody.velocity : base.cachedTransform.InverseTransformDirection(base.rigidbody.velocity)));
				if (!x.IsNone)
				{
					vector.x = x.Value;
				}
				...
				base.rigidbody.velocity = ((space == Space.World) ? vector : base.cachedTransform.TransformDirection(vector));
			}
		}
	}
}
