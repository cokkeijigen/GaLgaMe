...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformRotate : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DORotate(to.Value, duration.Value, rotateMode);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DORotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Rotate");
			}
			if (finishImmediately.Value)
			{
				Finish();
