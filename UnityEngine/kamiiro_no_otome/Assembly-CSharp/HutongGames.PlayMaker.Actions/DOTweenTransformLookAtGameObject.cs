...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLookAtGameObject : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLookAt(target.Value.transform.position, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Look At GameObject");
			}
			if (finishImmediately.Value)
			{
				Finish();
