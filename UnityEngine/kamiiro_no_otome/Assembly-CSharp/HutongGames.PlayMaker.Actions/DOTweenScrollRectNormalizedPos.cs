...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenScrollRectNormalizedPos : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<ScrollRect>().DONormalizedPos(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween ScrollRect NormalizedPos");
			}
			if (finishImmediately.Value)
			{
				Finish();
