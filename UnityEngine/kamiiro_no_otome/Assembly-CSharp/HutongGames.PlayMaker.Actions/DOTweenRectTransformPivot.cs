...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformPivot : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOPivot(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RectTransform Pivot");
			}
			if (finishImmediately.Value)
			{
				Finish();
