...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbody2DMoveY : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveY(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveY(toGameObject.Value.transform.position.y, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody2D Move Y");
			}
			if (finishImmediately.Value)
			{
				Finish();
