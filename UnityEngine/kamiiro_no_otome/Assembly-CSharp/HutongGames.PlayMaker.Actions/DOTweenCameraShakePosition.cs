...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraShakePosition : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOShakePosition(duration.Value, strength.Value, vibrato.Value, randomness.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Camera Shake Position");
			}
			if (finishImmediately.Value)
			{
				Finish();
