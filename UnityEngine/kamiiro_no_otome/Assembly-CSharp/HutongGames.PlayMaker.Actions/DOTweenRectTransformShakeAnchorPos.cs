...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformShakeAnchorPos : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOShakeAnchorPos(duration.Value, strength.Value, vibrato.Value, randomness.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RectTransform Shake Anchor Pos");
			}
			if (finishImmediately.Value)
			{
				Finish();
