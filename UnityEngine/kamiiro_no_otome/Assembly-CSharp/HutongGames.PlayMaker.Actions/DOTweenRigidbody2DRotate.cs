...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbody2DRotate : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DORotate(toAngle.Value, duration.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DORotate(toGameObject.Value.transform.rotation.z, duration.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody2D Rotate");
			}
			if (finishImmediately.Value)
			{
				Finish();
