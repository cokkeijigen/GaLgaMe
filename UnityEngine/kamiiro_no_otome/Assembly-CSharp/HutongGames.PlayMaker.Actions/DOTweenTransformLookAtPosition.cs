...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLookAtPosition : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLookAt(towards.Value, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Transform Look At Position");
			}
			if (finishImmediately.Value)
			{
				Finish();
