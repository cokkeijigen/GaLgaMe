...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyMoveZ : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case Target.Value:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMoveZ(to.Value, duration.Value, snapping.Value);
				break;
			case Target.GameObject:
				tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMoveZ(toGameObject.Value.transform.position.z, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Move Z");
			}
			if (finishImmediately.Value)
			{
				Finish();
