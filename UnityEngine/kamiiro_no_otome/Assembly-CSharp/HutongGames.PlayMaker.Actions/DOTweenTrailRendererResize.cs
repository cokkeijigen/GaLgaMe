...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTrailRendererResize : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<TrailRenderer>().DOResize(toStartWidth.Value, toEndWidth.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Trail Renderer Resize");
			}
			if (finishImmediately.Value)
			{
				Finish();
