...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyLookAtGameObject : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOLookAt(target.Value.transform.position, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RigidBody Look At GameObject");
			}
			if (finishImmediately.Value)
			{
				Finish();
