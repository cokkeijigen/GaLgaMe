...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformPivotX : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOPivotX(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween RectTransform PivotX");
			}
			if (finishImmediately.Value)
			{
				Finish();
