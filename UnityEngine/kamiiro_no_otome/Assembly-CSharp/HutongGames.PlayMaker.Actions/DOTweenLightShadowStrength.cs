...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLightShadowStrength : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOShadowStrength(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween Light Shadow Strength");
			}
			if (finishImmediately.Value)
			{
				Finish();
