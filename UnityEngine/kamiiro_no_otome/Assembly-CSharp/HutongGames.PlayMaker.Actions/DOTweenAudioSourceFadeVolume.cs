...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAudioSourceFadeVolume : FsmStateAction
	{
		...
		{
			tween = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<AudioSource>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				base.State.Debug("DOTween AudioSource Fade Volume");
			}
			if (finishImmediately.Value)
			{
				Finish();
